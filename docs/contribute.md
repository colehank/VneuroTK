...
1. **Write the changelog:**
   Create `CHANGELOG/<version>.md`. See previous entries for the format.
...
3. **Release:**
   ...
   > **Note:** The version is derived automatically from the git tag via `hatch-vcs`.
   > There is no need to edit `pyproject.toml`.
